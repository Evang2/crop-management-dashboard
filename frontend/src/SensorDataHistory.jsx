...
const SensorDataHistory = () => {
  ...
  const fetchHistoryData = async () => {
    try {
      setLoading(true);
      const response = await axios.get("http://127.0.0.1:5001/sensor_data_history");
  ...
  return (
    <div className="history-container">
      ...
      {loading ? (
        ...
        <p className="no-data-text">No historical data available for the selected time range.</p>
      ) : (
          ...
          <Card className="history-chart">
            ...
            <ResponsiveContainer width="100%" height={250}>
              <LineChart data={historyData}>
                ...
                <Tooltip />
                <Legend />
                <Line type="monotone" dataKey="soilMoisture" stroke="#3366cc" name="Soil Moisture (%)" />
              </LineChart>
            </ResponsiveContainer>
          </Card>
          ...
          <Card className="history-chart">
            ...
            <ResponsiveContainer width="100%" height={250}>
              <LineChart data={historyData}>
                ...
                <Tooltip />
                <Legend />
                <Line type="monotone" dataKey="N" stroke="#4CAF50" name="Nitrogen" />
                <Line type="monotone" dataKey="P" stroke="#9C27B0" name="Phosphorus" />
                <Line type="monotone" dataKey="K" stroke="#FF5722" name="Potassium" />
              </LineChart>
            </ResponsiveContainer>
          </Card>
        </div>
